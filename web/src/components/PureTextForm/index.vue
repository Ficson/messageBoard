  ...
  <el-form ref="form" class="page-form" :class="className" :model="data" :rules="rules" :label-width="labelWidth" :inline="true">
    <template v-for="(item, index) in getConfigList()">
      ...
      >
        ...
        <!-- 普通输入框 -->
        <span v-if="item.type === 'input' || item.type === 'password'" :style="`width: ${contentWidth};`">
          {{ data[item.value] }}
        </span>
        <!-- textarea -->
        <span v-if="item.type === 'textarea'" :style="`width: ${contentWidth};`">
          {{ data[item.value] }}
        </span>
        <!-- 选择框 -->
        <span v-if="item.type === 'select' && data[item.value]" :style="`width: ${contentWidth};`"
          >{{ listTypeInfo[item.list].find(li => li.value === data[item.value]).label }}
        </span>
        <!-- 日期选择框 -->
        <span v-if="item.type === 'date'" :style="`width: ${contentWidth};`"> {{ data[item.value] }} </span>
        <!-- 日期范围选择 -->
        <span v-if="item.type === 'datetimerange' && data[item.value]" :style="`width: ${contentWidth};`">
        ...
        <!-- 单选框 -->
        <span v-if="item.type === 'radio' && data[item.value]" :style="`width: ${contentWidth};`">
          {{ listTypeInfo[item.list].find(li => li.value === data[item.value]).label }}
        </span>
        <!-- 文本显示 -->
        ...
        <!-- 上传图片 -->
        <span v-if="item.type === 'upload'" class="avatar-uploader">
