  ...
  <div class="table-template">
    ...
    <div class="search-bar" v-if="queryOpts.length > 0">
      <el-form :inline="true" size="medium">
        <el-form-item v-for="(item, index) in queryOpts" :label="item.label" :key="index" style="margin-left: 10px">
          ...
          <el-button
            v-if="index === queryOpts.length - 1"
            style="margin-left: 20px"
            class="search-btn"
            type="primary"
            icon="el-icon-search"
            @click="loadData(pagination.pageSize, pagination.pageIndex)"
            >查询</el-button
          >
        </el-form-item>
      </el-form>
    ...
    <div class="table-top">
      ...
      <span style="float: right">
        共搜到
        <span style="color: #1b81c8">{{ pagination.total }}</span
        >条数据
      </span>
    </div>
    <el-table
    ...
    >
      ...
      <template v-for="(column, index) in columns">
        <el-table-column :prop="column.prop" :label="column.label" :align="column.align" :width="column.width" :key="column.prop">
        ...
        <template slot-scope="scope">
          <div class="operate-group">
            <template v-for="(btn, key) in operates.list">
              <div class="item" v-if="btn.show || (btn.showMethod && btn.showMethod(scope.$index, scope.row))" :key="key">
                <el-button
                  :type="btn.type"
                  size="mini"
                  :icon="btn.icon"
