...
router.post('/update', function (req, res, next) {
  userServ.updateUser(Object.assign(req.body, {id: req.userInfo.uid}), function (err, user) {
    if (err) return res.sendResult(null, 400, err)
...
router.get('/getUserInfo', function (req, res, next) {
  userServ.findById(req.userInfo.uid, function (err, user) {
    if (err) return res.sendResult(null, 400, err)
...
router.post('/findPage', function (req, res, next) {
  userServ.findPage(
    {key: req.query.username || '', pageIndex: parseInt(req.query.pageIndex) || 1, pageSize: parseInt(req.query.pageSize) || 10},
    function (err, data) {
      if (err) return res.sendResult(null, 400, err)
...
router.get('/findById', function (req, res, next) {
  userServ.findById(req.body.id, function (err, user) {
    if (err) return res.sendResult(null, 400, err)
    res.sendResult(user, 200, '获取成功')
  })
})
